...
internal static class AccessEndpoints
{
    ...
    public static IResult Login(UserDto login, [FromServices] IConfiguration config)
    {
        if (login.UserName == "test-user" && login.Password == "P@ssword")
        {
            ...
            // we define our token descriptor We need to utilise claims which are properties in our
            // token which gives information about the token which belong to the specific user who
            // it belongs to so it could contain their id, name, email the good part is that these
            // information are generated by our server and identity framework which is valid and trusted
            SecurityTokenDescriptor tokenDescriptor = new()
            {
              ...
              [
                ...
                // the life span of the token needs to be shorter and utilise refresh token to keep
                // the user signedin but since this is a demo app we can extend it to fit our
                // current need
                Expires = DateTime.UtcNow.AddHours(6),
                Audience = audience,
                Issuer = issuer,
                // here we are adding the encryption alogorithim information which will be used to
                // decrypt our token
